import math
from tracker_publisher.vive_pb2 import *
import time
import numpy as np
...
def tracker_infos_to_tf(global_msg):
    tf_msgs = []
    
    for tracker_id, tracker_data in global_msg['trackers'].items():
        

        tf_msg = TransformStamped()

        tf_msg.header = Header()
        timestamp_sec = int(global_msg['vive_timestamp'] / 1e9)
        tf_msg.header.stamp.sec = max(min(timestamp_sec, 2147483647), -2147483648)

        timestamp_nsec = int(global_msg['time_since_epoch'] % 1e9)
        tf_msg.header.stamp.nanosec = max(min(timestamp_nsec, 4294967295), 0)
        tf_msg.header.frame_id = "world_frame"

        tf_msg.child_frame_id = f'tracker_{tracker_id}'

        position = tracker_data['position']
        orientation = tracker_data['orientation']
